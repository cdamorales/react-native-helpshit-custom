...
RCT_EXPORT_METHOD(newConversationWithMessage:(NSString *)newConversationMessage messageError:(NSString *)messageError currentScreen:(NSString *)currentScreen)
{

    UIViewController *rootController = UIApplication.sharedApplication.delegate.window.rootViewController;
    HelpshiftAPIConfigBuilder *apiBuilder = [[HelpshiftAPIConfigBuilder alloc] init];
    apiBuilder.conversationPrefillText = newConversationMessage;
    NSString *messageErrorS = [NSString stringWithFormat:@"%@ %@ %@", messageError, @"en la pantalla", currentScreen];
    apiBuilder.customMetaData = [[HelpshiftSupportMetaData alloc] initWithMetaData:@{@"Error Técnico": messageErrorS}];
    HelpshiftAPIConfig *apiConfigObject = [apiBuilder build];
    [HelpshiftSupport showConversation:rootController withConfig:apiConfigObject];
}
